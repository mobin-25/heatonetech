...
export default function HomeView({ theme = 'dark' }: HomeViewProps) {
  ...
  // Auto-rotating banner slideshow
  ...
  // Stagger letter variants for name load animation
  const headingText = "HEAT ONE TECHNOLOGY";
  ...
  const letterVariants = {
    hidden: {
      ...
      y: 40,
      scale: 0.8,
      textShadow: "0px 0px 18px rgba(249, 115, 22, 0.9)",
    },
    visible: {
      opacity: 1,
      y: 0,
  ...
  return (
    <div className={`flex flex-col min-h-screen overflow-x-hidden animate-fadeIn ${
      ...
      {/* 1. HERO BANNER CONTAINER WITH PARALLAX EFFECT AND TEXT OVERLAY */}
      <div
        ...
        id="hero-banner-wrap"
      >
        {/* Animated Background Heating Rays - Dark mode only */}
        {theme === 'dark' && (
          <>
            <div className="absolute inset-0 bg-gradient-to-r from-orange-600/30 via-red-950/40 to-orange-600/30 mix-blend-color-burn z-10 pointer-events-none" />
            <div className="absolute inset-0 bg-black/60 z-10" />
          </>
        )}
        {/* Light mode: subtle dark vignette on bottom so pills/text pop */}
        {theme === 'light' && (
          <div className="absolute inset-0 bg-gradient-to-t from-slate-200/60 via-transparent to-white/20 z-10 pointer-events-none" />
        )}

        {/* Generated Image Background — rotating in light mode */}
        {theme === 'light' ? (
          <img
            key={bannerIdx}
            src={LIGHT_BANNERS[bannerIdx].src}
            alt={LIGHT_BANNERS[bannerIdx].label}
            className={`absolute inset-0 w-full h-full object-cover object-center translate-z-0 transition-opacity duration-700 ${
          ...
          <img
            ...
            className="absolute inset-0 w-full h-full object-cover opacity-80 translate-z-0"
            onError={(e) => { e.currentTarget.style.display = 'none'; }}
            id="hero-banner-image"
          />
        )}

        {/* Fallback CSS Glow Bars — only shown in dark mode */}
        {theme === 'dark' && (
          <div className="absolute inset-0 flex flex-col justify-center gap-12 px-12 opacity-40 select-none pointer-events-none">
            <div className="w-full h-4 bg-orange-500 rounded-full blur-sm opacity-80 animate-pulse shadow-[0_0_24px_#f97316]" />
            <div className="w-full h-4 bg-orange-500 rounded-full blur-md opacity-70 animate-pulse shadow-[0_0_36px_#f97316] delay-300" />
          </div>
        )}
        ...
        <div className="relative z-20 text-center px-4 w-full max-w-4xl" id="banner-text-overlay">
          ...
          >
            {/* Row 1: HEAT ONE */}
            <motion.div className={`flex flex-wrap justify-center gap-x-4 text-4xl md:text-7xl font-display font-bold uppercase tracking-wider ${
              theme === 'light' ? 'drop-shadow-[0_2px_6px_rgba(255,255,255,0.95)]' : 'drop-shadow-[0_4px_12px_rgba(0,0,0,0.9)]'
            }`}>
              {/* HEAT (white in dark, dark slate in light) */}
            ...
            {/* Row 2: TECHNOLOGY */}
            <motion.div className={`mt-3 text-lg md:text-4xl font-display uppercase tracking-[0.35em] md:tracking-[0.5em] font-semibold ${
              theme === 'light' 
                ? 'text-slate-800 drop-shadow-[0_1px_4px_rgba(255,255,255,0.9)]' 
                : 'text-[#8C827A] drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]'
            }`}>
          ...
          <motion.div 
            ...
            className="h-1 bg-orange-500 mx-auto mt-4 shadow-[0_0_12px_#ea580c]" 
            id="banner-underline"
          />
          ...
          <motion.p
            ...
            className="text-orange-500 font-mono text-xs uppercase tracking-[0.25em] mt-3 drop-shadow flex items-center justify-center gap-2"
          >
            <Sparkles className="w-3.5 h-3.5 text-orange-500 animate-pulse" />
            <span>Industrial Heating Solutions Products</span>
          </motion.p>

          {/* ADDED PRODUCTS DIRECTLY BELOW THE HEAT ONE TECHNOLOGY NAME */}
          <motion.div 
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {[
              "Mica Band", "Ceramic Band", "Cartridge Heaters", 
              "Ceramic Strip", "Tubular & Immersion", "Finned Air Heaters", 
              "Short Wave IR", "Ceramic IR", "IR Modules", 
              "Bobbin Heaters", "Micro Tubular", "Quartz Glass Tubes"
            ].map((prodName, idx) => (
              <span 
                key={idx}
                className={`text-[9px] md:text-[11px] font-mono font-medium border px-2.5 py-1 rounded-full drop-shadow-md hover:border-orange-500/80 transition-colors uppercase tracking-wider ${
                  theme === 'light'
                    ? 'border-slate-400/50 bg-white/80 backdrop-blur-sm text-slate-700'
                    : 'border-orange-500/30 bg-black/60 backdrop-blur-sm text-orange-300'
                }`}
              >
                {prodName}
              </span>
      ...
      {/* 2. ADVANCED HEATING SOLUTIONS INTRO - STRUCTURED BOX WITH GSTIN TAX CREDENTIAL */}
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-20" id="solutions-section">
        {/* Left Grid with Header */}
        ...
        {/* STRUCTURED ABOUT TEXT INSIDE THE CUSTOM INDUSTRIAL CARD BOX */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.7 }}
          className={`border rounded-xl p-6 md:p-8 relative overflow-hidden shadow-2xl ${
            theme === 'light' 
        ...
        >
          ...
          {/* Subtle Diagnostic Strip Header */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* Left Column: The Paragraph text beautifully structured in a dark plate */}
            ...
            {/* Right Column: GST No Orange Box & Address Credentials */}
            <div className="lg:col-span-5 flex flex-col gap-4">
              
              {/* GST NO BOX PRESENT IN THE SECOND IMAGE SCREENSHOT */}
              <div 
                className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-5 text-white flex flex-col justify-center relative overflow-hidden shadow-[0_4px_20px_rgba(249,115,22,0.3)] border border-orange-400/20"
                id="image-2-gst-block"
              >
                {/* Diagonal subtle lines decoration */}
              ...
              {/* Factoring Physical Locations from Page 4 Brochure */}
      ...
      {/* 3. KEY FACTS GRID WITH INTERACTIVE STATE */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6" id="facts-header-row">
            <div>
              ...
              <div className="h-1 w-16 bg-orange-500 mt-3 shadow-[0_0_8px_#ea580c] hidden md:block" />
            </div>

            {/* Fact Set Switches as shown in the two screenshots */}
            <div className={`flex items-center justify-center p-1 rounded-lg self-center md:self-end border ${
              ...
              <button
                onClick={() => {
                  setActiveFactSet('primary');
                  setSelectedFact(COMPANY_FACTS[1]); // DEFAULT SELECT LOCATION ON SWITCH
                }}
              ...
              <button
                onClick={() => {
                  setActiveFactSet('operational');
                  setSelectedFact(OTHER_FACTS[1]); // DEFAULT SELECT GST/TAXES ON SWITCH
                }}
                className={`px-4 py-2 text-xs font-semibold tracking-wider uppercase rounded-md transition-all duration-300 ${
          ...
          {/* Interactive grid split into cards on left, detail panel on right */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start" id="facts-interactive-interface">
              ...
              {getCurrentFacts().map((fact, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] md:text-xs font-mono tracking-widest text-[#9ca3af]/80 uppercase mb-1 block group-hover:text-amber-200 transition-colors">
                      {fact.title}
                    </span>
                    
                    <span className={`text-sm md:text-base font-semibold group-hover:text-white transition-colors uppercase ${
            ...
            {/* Detail panel updating dynamically */}
            <div className="lg:col-span-4" id="facts-detail-panel">
              <motion.div 
                ...
                className="bg-[#0b0c0e] border border-orange-500/10 rounded-xl p-6 md:p-8 shrink-0 relative overflow-hidden flex flex-col justify-between shadow-2xl min-h-[340px]"
                id="facts-detail-card"
              >
                {/* Background glowing sienna circle */}
                ...
                {selectedFact?.id === 'location' ? (
                  <a
                    ...
                    className="p-4 bg-[#111215] hover:bg-zinc-900 border border-orange-500/30 hover:border-orange-500 rounded-lg flex items-center justify-between group/metric select-none cursor-pointer transition-colors"
                  ...
                  >
                    <div>
                      ...
                      <span className="text-xs uppercase font-medium text-zinc-100 group-hover:text-orange-400 font-sans transition-colors">Open in Google Maps ↗</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-right">
                      <span className="text-sm md:text-base font-semibold font-mono text-orange-400 group-hover:text-orange-500 transition-colors">1 Modern Road No 26</span>
      ...
      {/* 4. TECHNICAL ADHESION VALUE STATS */}
